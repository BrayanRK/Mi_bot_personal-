...
async function getVideoLink(videoUrl) {
  ...
  const res = await axios.get(`${DELIRIUS_BASE}/ytmp4`, {
    params: { url: cleanUrl, format: VIDEO_QUALITY },
    timeout: 60000,
    validateStatus: () => true,
  ...
  // Cubrir múltiples estructuras posibles de delirius
  const dlUrl =
    d?.data?.url ||
    d?.data?.download ||
    d?.data?.video ||
  ...
  const title = safeFileName(d?.data?.title || d?.title || "video");
  const thumbnail =
...
⏳ Espera un momento...`, msg);
      ...
      if (!thumbnail && link.thumbnail) thumbnail = link.thumbnail;

      const videoInfo  = await downloadVideo(link.dlUrl, rawFile);
      const finalTitle = safeFileName(title);
