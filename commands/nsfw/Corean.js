...
export default {
  name: "corean",
  aliases: ["coreana", "kgirl"],
  run: async (sock, msg, args, jid) => {
    try {
      if (!isNsfwEnabled(jid))
        return reply(sock, jid, "🚫 El contenido NSFW está desactivado.\n\nUn admin debe usar *.nsfw on*", msg);

      const response = await axios.get("https://api.delirius.store/nsfw/corean", { responseType: "arraybuffer" });
      await sock.sendMessage(jid, { image: Buffer.from(response.data), caption: "Coreana 🔞" }, { quoted: msg });
    } catch (e) {
